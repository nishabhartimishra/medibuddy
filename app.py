...
import numpy as np
import torch
from diabetes_model import ANN_Model
...
def predict(values, dic):
    ...
    if len(values) == 8:
        ...
        values = np.asarray(values2, dtype=np.float32)
        with torch.no_grad():
            output = model.forward(torch.tensor(values).unsqueeze(0))
...
def predictPage():
    ...
    return render_template('diabetes_result.html', pred=pred)
